...
void* get_bool_constants(void*)
{
	...
	k->AllNpcsHaveRadar.scan(OBFUSCATE("kAllNpcsHaveRadar"));
	k->AwfulMode.scan(OBFUSCATE("kAwfulMode"));
	k->AwfulMode2.scan(OBFUSCATE("kAwfulMode2"));
	k->OrbitMode.scan(OBFUSCATE("kOrbitMode"));
	k->SimulateNoConnection.scan(OBFUSCATE("kSimulateNoConnection"));
	k->UseFakeExternalFriends.scan(OBFUSCATE("kUseFakeExternalFriends"));
	k->DrawSunLighting.scan(OBFUSCATE("kDrawSunLighting"));
	k->DisableEnvironment.scan(OBFUSCATE("kDisableEnvironment"));
	k->AudienceShowFriendNames.scan(OBFUSCATE("kAudienceShowFriendNames"));
	k->AppleTVMode.scan(OBFUSCATE("kAppleTVMode"));
	k->LogCheckpoint.scan(OBFUSCATE("kLogCheckpoint"));
	k->EnableFluffyClouds.scan(OBFUSCATE("kEnableFluffyClouds"));
	k->SuperSlidey.scan(OBFUSCATE("kSuperSlidey"));
	k->ForceNativeRes.scan(OBFUSCATE("kForceNativeRes"));
	k->SerendipityTagTransDur.scan(OBFUSCATE("kSerendipityTagTransDur"));
	k->ForceUnclampTime.scan(OBFUSCATE("kForceUnclampTime"));
	k->MaxBounceOffset.scan(OBFUSCATE("kMaxBounceOffset"));
	k->MinBounceOffset.scan(OBFUSCATE("kMinBounceOffset"));
	k->ShadowBrightAlpha.scan(OBFUSCATE("kShadowBrightAlpha"));
	k->PongFadeOutDur.scan(OBFUSCATE("kPongFadeOutDur"));
	k->PongPauseDur.scan(OBFUSCATE("kPongPauseDur"));
	k->PongFadeInDur.scan(OBFUSCATE("kPongFadeInDur"));
	k->SerendipityTagScale.scan(OBFUSCATE("kSerendipityTagScale"));
	k->WsOffset.scan(OBFUSCATE("kWsOffset"));
	k->NormPosScalar.scan(OBFUSCATE("kNormPosScalar"));
	k->TagIconOffset.scan(OBFUSCATE("kTagIconOffset"));
	k->ArrowOffsetScalar.scan(OBFUSCATE("kArrowOffsetScalar"));
	k->ArrowScale.scan(OBFUSCATE("kArrowScale"));
	k->GradientFacebook.scan(OBFUSCATE("kGradientFacebook"));
	k->GradientAlpha.scan(OBFUSCATE("kGradientAlpha"));
	k->GradientScale.scan(OBFUSCATE("kGradientScale"));
	k->TguiStateCounter.scan(OBFUSCATE("kTguiStateCounter"));
	k->LumHighThresh.scan(OBFUSCATE("kLumHighThresh"));
	k->RayTop.scan(OBFUSCATE("kRayTop"));
	k->RayMid.scan(OBFUSCATE("kRayMid"));
	k->GlowMid.scan(OBFUSCATE("kGlowMid"));
	k->GlowTop.scan(OBFUSCATE("kGlowTop"));
	k->NumberOffsetX.scan(OBFUSCATE("kNumberOffsetX"));
	k->NumberOffsetY.scan(OBFUSCATE("kNumberOffsetY"));
	k->FakeWaxMultiplier.scan(OBFUSCATE("kFakeWaxMultiplier"));
	k->EnableFakeWaxMultiplier.scan(OBFUSCATE("kEnableFakeWaxMultiplier"));
	k->KeepCornerButtonsVisible.scan(OBFUSCATE("kKeepCornerButtonsVisible"));
	k->ForceMagnify.scan(OBFUSCATE("kForceMagnify"));
	k->HideNumber.scan(OBFUSCATE("kHideNumber"));
	k->DebugHudOffsetsCandle.scan(OBFUSCATE("kDebugHudOffsetsCandle"));
	k->BgSize.scan(OBFUSCATE("kBgSize"));
	k->BgOffsetX.scan(OBFUSCATE("kBgOffsetX"));
	k->BgOffsetY.scan(OBFUSCATE("kBgOffsetY"));
	k->BgAlpha.scan(OBFUSCATE("kBgAlpha"));
	k->BadgeOffsetX.scan(OBFUSCATE("kBadgeOffsetX"));
	k->BadgeOffsetY.scan(OBFUSCATE("kBadgeOffsetY"));
	k->ChevronSize.scan(OBFUSCATE("kChevronSize"));
	k->ChevronMargin.scan(OBFUSCATE("kChevronMargin"));
	k->CandleScale.scan(OBFUSCATE("kCandleScale"));
	k->DropShadowA.scan(OBFUSCATE("kDropShadowA"));
	k->EnableVideoRecorderUI.scan(OBFUSCATE("kEnableVideoRecorderUI"));
	k->MaxVideoRecordSeconds.scan(OBFUSCATE("kMaxVideoRecordSeconds"));
	k->UseFixedTimestepRecording.scan(OBFUSCATE("kUseFixedTimestepRecording"));
	k->DisplayStarIndex.scan(OBFUSCATE("kDisplayStarIndex"));
	k->FlameToCandleScale.scan(OBFUSCATE("kFlameToCandleScale"));
	k->EnableQuickMenu.scan(OBFUSCATE("kEnableQuickMenu"));
	k->Invincibility.scan(OBFUSCATE("kInvincibility"));
	k->EnableAllRelationshipAbilities.scan(OBFUSCATE("kEnableAllRelationshipAbilities"));
	k->BlackWhite.scan(OBFUSCATE("kBlackWhite"));
	k->ShoutAutoRelease.scan(OBFUSCATE("kShoutAutoRelease"));
	k->Disable_Avatar.scan(OBFUSCATE("kDisable_Avatar"));

	k->ConstellationGateDisable.scan(OBFUSCATE("kConstellationGateDisable"));
	k->AutoSkipAllTimelines.scan(OBFUSCATE("kAutoSkipAllTimelines"));
	k->FlameInHand.scan(OBFUSCATE("kFlameInHand"));
	k->AvatarCharcoaling.scan(OBFUSCATE("kAvatarCharcoaling"));
	k->DisableGiftRelationshipTimeCheck.scan(OBFUSCATE("kDisableGiftRelationshipTimeCheck"));
	k->DisableWind.scan(OBFUSCATE("kDisableWind"));
	k->ApplyForcesInFlightComputer.scan(OBFUSCATE("kApplyForcesInFlightComputer"));
	k->EnableMultiplayer.scan(OBFUSCATE("kEnableMultiplayer"));
	k->FishSchoolDebug.scan(OBFUSCATE("kFishSchoolDebug"));
	k->Disable_Hud.scan(OBFUSCATE("kDisable_Hud"));
	k->Disable_Ocean.scan(OBFUSCATE("kDisable_Ocean"));
	k->AllowOverride.scan(OBFUSCATE("kAllowOverride"));
	k->CheapFogEnabled.scan(OBFUSCATE("kCheapFogEnabled"));
	k->FakeCapeLevelEnable.scan(OBFUSCATE("kFakeCapeLevelEnable"));
	k->RunSpeed.scan(OBFUSCATE("kRunSpeed"));
	k->FakeCapeLevel.scan(OBFUSCATE("kFakeCapeLevel"));
	k->LiftUpForceMultiplier.scan(OBFUSCATE("kLiftUpForceMultiplier"));
	k->CheapFogHeight.scan(OBFUSCATE("kCheapFogHeight"));
	k->CheapFogHeightDensity.scan(OBFUSCATE("kCheapFogHeightDensity"));
